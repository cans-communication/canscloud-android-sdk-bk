...
class CorePreferences constructor(private val context: Context) {
    ...
    var debugLogs: Boolean
        get() = config.getBool("app", "debug", com.cans.canscloud_android_sdk.BuildConfig.DEBUG)
        set(value) {
...
//        set(value) = config.setString("app", "device_name", value)
    ...
    private fun copy(from: String, to: String, overrideIfExists: Boolean = false) {
        ...
        if (outFile.exists()) {
            if (!overrideIfExists) {
                android.util.Log.i(context.getString(com.cans.canscloud_android_sdk.R.string.app_name), "[Preferences] File $to already exists")
                return
            }
        }
        android.util.Log.i(context.getString(com.cans.canscloud_android_sdk.R.string.app_name), "[Preferences] Overriding $to by $from asset")
    ...
    private fun move(from: String, to: String, overrideIfExists: Boolean = false) {
        ...
        if (inFile.exists()) {
            if (outFile.exists() && !overrideIfExists) {
                android.util.Log.w(context.getString(com.cans.canscloud_android_sdk.R.string.app_name), "[Preferences] Can't move [$from] to [$to], destination file already exists")
            } else {
                ...
                inFile.delete()
                android.util.Log.i(context.getString(com.cans.canscloud_android_sdk.R.string.app_name), "[Preferences] Successfully moved [$from] to [$to]")
            }
        } else {
            android.util.Log.w(context.getString(com.cans.canscloud_android_sdk.R.string.app_name), "[Preferences] Can't move [$from] to [$to], source file doesn't exists")
        }
    }
}
